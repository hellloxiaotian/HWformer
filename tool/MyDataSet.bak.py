...
class Multi_task_Art_nosie_Dataset(Dataset):   # 添加人工噪声
    ...
    def __getitem__(self, index):
        ...
        if self.name_list is None:
            if self.args.task == "denoise":
                return [clean_img, clean_img, clean_img], nos_img
            else:
                raise ValueError("Please choose the args.task value: denoise, derainy, denoise+derainy")
        else:
            return clean_img, nos_img, self.name_list[index]
