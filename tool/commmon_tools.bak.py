    ...
    def train(data_loader, model, loss_f, optimizer, epoch_id, device, max_epoch, writer=None):
        ...
        for n_count, data in enumerate(data_loader):
            ...
            if isinstance(outputs, list):
                # assert len(outputs) == len(loss_f)
                # loss = (loss_f[0](outputs[0], ori_img[0]) + loss_f[1](outputs[1], ori_img[1]) + loss_f[2](outputs[2], ori_img[2])) / (ori_img[0].size()[0] * 2)
                if isinstance(loss_f, list):
                    assert len(outputs) == len(loss_f)
                    loss = (loss_f[0](outputs[0], ori_img[0]) + loss_f[1](outputs[1], ori_img[1]) + loss_f[2](outputs[2], ori_img[2])) / (ori_img[0].size()[0] * 2)
                else:
                    # loss = (loss_f(outputs[0], ori_img[0]) + loss_f(outputs[1], ori_img[1]) + loss_f(outputs[2], ori_img[2])) / (ori_img[0].size()[0] * 2)
                    # loss = (loss_f(outputs[0], ori_img[0]) +
                    #         loss_f(outputs[1], ori_img[0]) +
                    #         loss_f(outputs[2], ori_img[0])) / \
                    #        (ori_img[0].size()[0] * 2)
