...
class ModelTrainer(object):
    ...
    @staticmethod
    def train_mt(data_loader, model, loss_f, optimizer, epoch_id, device, max_epoch, writer=None):
        loss_sigma = []
        ...
        for n_count, data in enumerate(data_loader):
            ...
            # loss = loss_f(outputs, ori_img) / (ori_img.size()[0] * 2)
            if len(outputs) == 2:
                loss = (loss_f(outputs[0], ori_img) + loss_f(outputs[1], ori_img)) / (ori_img.size()[0] * 2)
            else:
